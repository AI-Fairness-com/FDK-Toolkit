...
# ================================================================
# HIRING-SPECIFIC REPORT GENERATION WITH DATASET OVERVIEW
# ================================================================
...
def build_hiring_summaries(audit: dict) -> list:
    ...
    if "validation" in audit:
        ...
        lines.append(f"   → Total Candidates Analyzed: {validation_info.get('sample_size', 'N/A')}")
        lines.append(f"   → Candidate Groups: {validation_info.get('groups_analyzed', 'N/A')}")
        ...
        lines.append(f"   → Total Candidates Analyzed: {total_records}")
        lines.append(f"   → Candidate Groups: {num_groups}")
        if num_groups <= 10:
            lines.append(f"   → Group Distribution: {dict(group_counts)}")
        else:
            lines.append(f"   → Largest Group: {max(group_counts.values())} candidates")
            lines.append(f"   → Smallest Group: {min(group_counts.values())} candidates")
    else:
...
# ================================================================
# HIRING ROUTES DEFINITION (BLUEPRINT COMPATIBLE)
# ================================================================

@hiring_bp.route('/hiring-upload')
...
def run_hiring_audit_with_mapping():
    ...
    try:
        ...
        # Generate hiring-specific human-readable summary with Dataset Overview
