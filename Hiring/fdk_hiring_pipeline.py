...
# 18 Comprehensive Hiring Fairness Metrics
...
class HiringFairnessPipeline:
    ...
    def convert_numpy_types(self, obj):
        ...
        if isinstance(obj, (np.integer, np.int8, np.int16, np.int32, np.int64)):
            ...
            # ✅ CRITICAL FIX: Convert ALL keys to strings
            converted_dict = {}
            for key, value in obj.items():
                # Convert key through same process to handle nested numpy types
                converted_key = self.convert_numpy_types(key)
                # Ensure key is string for JSON
    ...
    def calculate_core_group_fairness(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for group in groups:
            ...
            # 1. Selection Rate (Statistical Parity components)
    ...
    def calculate_equality_opportunity_treatment(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # 5. Equal Opportunity Difference and Equalized Odds
        ...
        # 7. Treatment Equality (FNR-FPR Ratio)
    ...
    def calculate_error_prediction_fairness(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for group in groups:
            ...
            try:
                ...
                # 10. Predictive Parity components
    ...
    def calculate_individual_fairness_consistency(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # 11. Individual Consistency Index
        numeric_cols = df.select_dtypes(include=[np.number]).columns
        numeric_cols = [col for col in numeric_cols if col not in ['y_true', 'y_pred', 'y_prob', 'group']]
        
        if len(numeric_cols) > 0:
            try:
                # Use nearest neighbors to measure consistency
                X = df[numeric_cols].values
                nbrs = NearestNeighbors(n_neighbors=5, algorithm='ball_tree').fit(X)
                distances, indices = nbrs.kneighbors(X)
                
                # Calculate prediction consistency among neighbors
                consistency_scores = []
    ...
    def calculate_subgroup_bias_detection(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # 13. MDSS Subgroup Discovery Score and Error Disparity
        error_rates = {}
        for group in groups:
            group_mask = df['group'] == group
            group_data = df[group_mask]
    ...
    def calculate_robustness_temporal_fairness(self, df: pd.DataFrame, all_metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 17. Worst-Group Accuracy and Composite Bias Score
        accuracies = {}
        for group in groups:
            group_mask = df['group'] == group
            group_data = df[group_mask]
    ...
    def calculate_all_metrics(self, df: pd.DataFrame) -> Dict[str, Any]:
        """Calculate all 18 hiring fairness metrics"""
        metrics = {}
...
if __name__ == "__main__":
    ...
    sample_data = pd.DataFrame({
        ...
        'experience': [5, 3, 4, 6, 5, 4],
        'education': [16, 14, 18, 16, 17, 15],
    ...
    print("PRODUCTION HIRING FAIRNESS AUDIT COMPLETE")
    print(f"Metrics Calculated: {results['metrics_calculated']}/18")
