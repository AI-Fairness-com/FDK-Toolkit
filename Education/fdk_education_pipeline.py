# ================================================================
# FDK Education Pipeline 
# ================================================================

import pandas as pd
import numpy as np
from sklearn.metrics import accuracy_score, confusion_matrix, roc_auc_score, precision_score, recall_score, brier_score_loss
...
# ================================================================
# METRICS CONFIGURATION - 26 VALIDATED METRICS
# ================================================================
...
EDUCATION_METRICS_CONFIG = {
    ...
    'educational_specific_metrics': [
        ...
        'learning_trajectory_fairness',
        'opportunity_gap_metric'
    ],
    ...
    'temporal_stability_metrics': [
        'fairness_drift_index'
    ],
    'explainability_metrics': [
        'feature_attribution_parity'
    ],
...
# ================================================================
# EDUCATIONAL SPECIFIC METRICS (8 METRICS)
# ================================================================

def calculate_educational_specific_metrics(df: pd.DataFrame) -> Dict[str, Any]:
    """Calculate 8 educational domain-specific metrics"""
    metrics = {}
    groups = df['group'].unique()
    ...
    # 15. Learning Trajectory Fairness
    if 'progress_score' in df.columns:
        progress_variances = []
        for group in groups:
            group_mask = df['group'] == group
            progress_variance = df[group_mask]['progress_score'].var()
            if not np.isnan(progress_variance):
                progress_variances.append(progress_variance)
        
        if len(progress_variances) >= 2:
            metrics['learning_trajectory_fairness'] = float(max(progress_variances) - min(progress_variances))
        else:
            metrics['learning_trajectory_fairness'] = 0.0
    else:
        metrics['learning_trajectory_fairness'] = 0.0
    
    # 16. Opportunity Gap Metric
    resource_cols = [col for col in df.columns if 'resource' in col.lower() or 'access' in col.lower()]
    if resource_cols:
        resource_gaps = []
        for col in resource_cols[:2]:  # Limit to top 2 resource columns
            if pd.api.types.is_numeric_dtype(df[col]):
                group_means = [df[df['group'] == g][col].mean() for g in groups]
                if len(group_means) >= 2:
                    resource_gap = max(group_means) - min(group_means)
                    resource_gaps.append(resource_gap)
        
        if resource_gaps:
            metrics['opportunity_gap_metric'] = float(np.mean(resource_gaps))
        else:
            metrics['opportunity_gap_metric'] = 0.0
    else:
        metrics['opportunity_gap_metric'] = 0.0
...
# ================================================================
# CALIBRATION AND RELIABILITY METRICS (2 METRICS)
# ================================================================

def calculate_calibration_reliability_metrics(df: pd.DataFrame) -> Dict[str, Any]:
    """Calculate 2 calibration and reliability metrics"""
    metrics = {}
    groups = df['group'].unique()
    ...
    for group in groups:
        ...
        if len(group_data) == 0 or 'y_prob' not in group_data.columns:
            continue
        ...
        try:
            ...
            calibration_error = np.mean(np.abs(y_prob - y_true))
            calibration_scores[group] = float(calibration_error)
...
# ================================================================
# INDIVIDUAL FAIRNESS METRICS (2 METRICS)
# ================================================================

def calculate_individual_fairness_metrics(df: pd.DataFrame) -> Dict[str, Any]:
    """Calculate 2 individual fairness metrics"""
    metrics = {}
            ...
            for i in range(len(df_sample)):
                ...
                for j in range(len(df_sample)):
                    if i != j:
                        ...
                        for col in feature_cols[:3]:  # Limit to top 3 features
                            if pd.api.types.is_numeric_dtype(df_sample[col]):
                                val_i = df_sample.iloc[i][col]
                                val_j = df_sample.iloc[j][col]
                                if not (np.isnan(val_i) or np.isnan(val_j)):
                                    col_range = df_sample[col].max() - df_sample[col].min()
                                    if col_range > 0:
                                        feature_similarity -= abs(val_i - val_j) / (3 * col_range)
                        
                        # Calculate prediction similarity
                        pred_i = df_sample.iloc[i]['y_pred']
                        pred_j = df_sample.iloc[j]['y_pred']
                        pred_similarity = 1 - abs(pred_i - pred_j)
                        
                        consistency = abs(feature_similarity - pred_similarity)
                        similarities.append(consistency)
                
                if similarities:
                    consistency_scores.append(np.mean(similarities))
            
            if consistency_scores:
                metrics['individual_fairness_consistency'] = float(1 - np.mean(consistency_scores))
        
        except Exception:
            metrics['individual_fairness_consistency'] = 0.5
    
    # Counterfactual Explanation Fairness
    groups = df['group'].unique()
    if len(groups) >= 2 and len(feature_cols) > 0:
        explanation_disparities = []
        for feature in feature_cols[:2]:  # Limit to top 2 features
            if pd.api.types.is_numeric_dtype(df[feature]):
                feature_correlations = []
                for group in groups:
                    group_mask = df['group'] == group
                    correlation = df[group_mask][feature].corr(df[group_mask]['y_pred'])
                    if not np.isnan(correlation):
                        feature_correlations.append(abs(correlation))
                
                if len(feature_correlations) >= 2:
                    disparity = max(feature_correlations) - min(feature_correlations)
                    explanation_disparities.append(disparity)
        
        if explanation_disparities:
            metrics['counterfactual_explanation_fairness'] = float(1 - np.mean(explanation_disparities))
        else:
            metrics['counterfactual_explanation_fairness'] = 0.5
    else:
        metrics['counterfactual_explanation_fairness'] = 0.5
...
# ================================================================
# TEMPORAL STABILITY METRICS (1 METRIC)
# ================================================================
...
def calculate_explainability_metrics(df: pd.DataFrame) -> Dict[str, Any]:
    """Calculate 1 explainability metric"""
    metrics = {}
    groups = df['group'].unique()
    
    feature_cols = [col for col in df.columns if col not in ['group', 'y_true', 'y_pred', 'y_prob']]
    if len(feature_cols) > 0 and len(groups) >= 2:
        feature_attributions = {}
        
        for feature in feature_cols[:3]:  # Limit to top 3 features
            if pd.api.types.is_numeric_dtype(df[feature]):
                group_correlations = {}
                for group in groups:
                    group_mask = df['group'] == group
                    correlation = df[group_mask][feature].corr(df[group_mask]['y_pred'])
                    if not np.isnan(correlation):
                        group_correlations[group] = abs(correlation)
                
                if group_correlations:
                    feature_attributions[feature] = group_correlations
        
        if feature_attributions:
            attribution_disparities = []
            for feature, group_corrs in feature_attributions.items():
                if len(group_corrs) >= 2:
                    disparity = max(group_corrs.values()) - min(group_corrs.values())
                    attribution_disparities.append(disparity)
            
            if attribution_disparities:
                metrics['feature_attribution_parity'] = float(1 - np.mean(attribution_disparities))
            else:
                metrics['feature_attribution_parity'] = 1.0
        else:
            metrics['feature_attribution_parity'] = 1.0
    else:
        metrics['feature_attribution_parity'] = 1.0
...
def calculate_validation_robustness_metrics(df: pd.DataFrame, core_metrics: Dict) -> Dict[str, Any]:
    ...
    # 23. Worst Case Subgroup Performance
    ...
    # 24. Cross-Validation Fairness Consistency
    ...
    # 25. Temporal Fairness Stability
    bootstrap_stability = []
    n_bootstraps = 5
...
class EducationFairnessPipeline:
    """Enhanced education fairness pipeline with temporal tracking"""
    ...
    def run_pipeline(self, df: pd.DataFrame, save_to_disk: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            print("⚖️ Calculating calibration metrics...")
            calibration_metrics = calculate_calibration_reliability_metrics(df_clean)
            
            print("👤 Calculating individual fairness metrics...")
            individual_metrics = calculate_individual_fairness_metrics(df_clean)
            
            print("🕰️ Calculating temporal stability metrics...")
            temporal_metrics = calculate_temporal_stability_metrics(df_clean, self.metrics_history)
            
            print("🔎 Calculating explainability metrics...")
            explainability_metrics = calculate_explainability_metrics(df_clean)
            ...
            # Combine all metrics
            all_metrics = {
                **core_metrics, **educational_metrics, **calibration_metrics,
            ...
            self.metrics_history.append(all_metrics.copy())
...
def identify_critical_issues(metrics: Dict) -> List[str]:
    ...
    if metrics.get('expected_calibration_error', 0) > 0.1:
        issues.append("Poor model calibration reliability across student groups")
    
    if metrics.get('individual_fairness_consistency', 0.5) < 0.7:
        issues.append("Inconsistent treatment of similar students")
    
    if metrics.get('fairness_drift_index', 1.0) < 0.8:
        issues.append("Significant fairness degradation over time detected")
...
def run_audit_from_request(audit_request: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        pipeline = EducationFairnessPipeline()
        results = pipeline.run_pipeline(df, save_to_disk=False)
...
if __name__ == "__main__":
    ...
    test_data = pd.DataFrame({
        ...
        'progress_score': np.random.normal(0, 1, n_samples),
        'resource_access': np.random.choice([1, 2, 3], n_samples, p=[0.3, 0.5, 0.2]),
    ...
    def biased_predictions(row):
        ...
        if row['group'] == 'Group_A':
            return 1 if base_prob > 0.3 else 0
        elif row['group'] == 'Group_B': 
            return 1 if base_prob > 0.5 else 0
        else:
            return 1 if base_prob > 0.7 else 0
    
    test_data['y_pred'] = test_data.apply(biased_predictions, axis=1)
    
    print("🧪 Testing Enhanced Education Fairness Pipeline...")
    ...
    print("\n📊 ENHANCED EDUCATION FAIRNESS AUDIT RESULTS:")
    ...
    print(f"Metrics Calculated: {results['metrics_calculated']}/26")
