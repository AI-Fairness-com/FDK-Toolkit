...
def _build_professional_summary(audit_response: dict, composite_score: float, fairness_metrics: dict) -> list:
    ...
    # Extract dataset statistics from the actual audit response structure
    data_validation = audit_response.get('data_validation', {})
    selection_rates = fairness_metrics.get('selection_rates', {})
    predicted_positives = fairness_metrics.get('predicted_positives_per_group', {})
    predicted_negatives = fairness_metrics.get('predicted_negatives_per_group', {})
    
    if selection_rates and predicted_positives:
        group_counts = len(selection_rates)
        total_patients = sum(predicted_positives.values()) + sum(predicted_negatives.values())
        min_rate = min(selection_rates.values()) if selection_rates else 0
        max_rate = max(selection_rates.values()) if selection_rates else 0
        
        lines.append(f"   → Patient Groups Analyzed: {group_counts}")
        lines.append(f"   → Total Patients: {total_patients}")
        lines.append(f"   → Selection Rate Range: {min_rate:.3f} - {max_rate:.3f}")
        
        # Add base rates if available
        base_rates = fairness_metrics.get('base_rates', {})
        if base_rates:
            min_base = min(base_rates.values()) if base_rates else 0
            max_base = max(base_rates.values()) if base_rates else 0
            lines.append(f"   → Base Rate Range: {min_base:.3f} - {max_base:.3f}")
    
    elif data_validation:
        # Use validation info if available
        warnings = data_validation.get('warnings', [])
        clinical_concerns = data_validation.get('clinical_concerns', [])
        
        if warnings:
            # Show first warning only to avoid clutter
            first_warning = warnings[0] if warnings else ""
            if "Small group sizes" in first_warning:
                lines.append("   → Note: Some patient groups have limited sample sizes")
            else:
                lines.append(f"   → Data Note: {first_warning[:100]}...")
        
        if clinical_concerns:
            lines.append(f"   → Clinical Review: {clinical_concerns[0]}")
        
        lines.append("   → Dataset validation completed successfully")
